...
{
    class describe_BaseControllerExtended : nspec
    {
        private static CultureInfo EN_US = new CultureInfo("en-US");
        private static CultureInfo CS_CZ = new CultureInfo("cs-CZ");

        BaseController2 controller;

        public class BaseController2 : BaseController
        {
            public override CultureInfo GetInvariantCulture()
            {
                return EN_US;
            }
        }
        ...
        void before_each()
        {
            controller = new BaseController2();
        }
